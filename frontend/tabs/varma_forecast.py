...
def varma_forecast_tab(file_path, interval, target_column, prediction_ahead, lookback_days):
    st.header(f"VARMA Forecast ({interval})")
    ...
    try:
        ...
        if st.button("Predict with VARMA"):
            with st.spinner("Making VARMA predictions..."):
                # Get predictions
                ...
                st.session_state['model_results']['VARMA']['forecast'] = forecast
                st.session_state['model_results']['VARMA']['future_index'] = future_index
                ...
                fig = px.line(combined_df, x='Date', y='Price', color='Type', title=f'VARMA Forecast for {target_column} ({interval})')
                ...
                st.download_button("Download Forecast", csv, f"varma_forecast_{interval}.csv", "text/csv")

    except Exception as e:
        st.error(f"Error generating VARMA forecast for {interval}: {str(e)}")