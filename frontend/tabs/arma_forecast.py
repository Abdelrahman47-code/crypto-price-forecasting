...
from utils.data_utils import get_yfinance_data, check_stationarity, calculate_metrics
from backend.predictor import Predictor

def arma_forecast_tab(crypto_symbol, period, interval, target_column, prediction_ahead):
    st.header("ARMA Forecasting Model")
    predictor = Predictor()
    predictor.load_models()

    if st.button("Predict with ARMA"):
        with st.spinner("Making ARMA predictions..."):
            btc_data = get_yfinance_data(crypto_symbol, period, interval)
            if btc_data is not None:
                is_stationary, _, _ = check_stationarity(btc_data, column=target_column)
                if not is_stationary:
                    btc_data[target_column] = btc_data[target_column].diff().dropna()
                ...
                forecast = predictor.get_predictions(
                    ...
                    model_type="arima"  # ARMA is a special case of ARIMA with d=0
                )
                ...
                st.session_state['model_results']['ARMA'] = {
                    'forecast': forecast,
                ...
                plt.title(f'{crypto_symbol} ARMA Model Predictions ({target_column})')
                plt.xlabel('Date')
